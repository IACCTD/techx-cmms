...
const DB = (() => {
  ...
  const EMPTY = { assets: [], pms: [], parts: [], wos: [], meta: { site: 'IAC Cottondale, AL' } };
  ...
  function load() {
    ...
    try {
      ...
      cache = raw ? Object.assign({}, EMPTY, JSON.parse(raw)) : structuredClone(EMPTY);
    } catch (e) {
...
const Backup = {
  export() {
    ...
    if (window.toast) toast('Backup downloaded');
  },
  import(file, done) {
    const r = new FileReader();
    r.onload = () => {
      try {
        DB.replaceAll(JSON.parse(r.result));
        done(null);
      } catch (e) { done(e); }
    };
    r.readAsText(file);
