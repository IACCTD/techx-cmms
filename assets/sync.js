  ...
  async pull(mode = 'replace') {
    ...
    if (mode === 'replace') {
      ...
      shared.meta = Object.assign({}, shared.meta, {
        ...
        recentAssets: local.meta.recentAssets || []
      });
    ...
    cols.forEach(c => {
      ...
      theirs.forEach(r => {
        ...
        else if (mode === 'repo') { byId.set(r.id, Object.assign({}, byId.get(r.id), r)); stats.updated++; }
        else { byId.set(r.id, Object.assign({}, r, byId.get(r.id))); stats.kept++; }
      });
