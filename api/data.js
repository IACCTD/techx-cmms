/* ============================================================
   ...
   Note: @vercel/postgres is no longer maintained — Vercel's own
   Postgres product is now Neon, so this uses the Neon driver.

   Accounts:
     - Each person has their own username and password.
     - Passwords are stored as PBKDF2-SHA256 hashes (210k rounds)
       with a per-user random salt. Plain text is never stored,
       never logged, and never sent back to the browser.
     - Sign-in returns a random session token, stored server-side.
       The browser only ever holds the token, not the password.

   Roles:
     - maintenance : create/edit assets, PMs, parts, work orders
     - admin       : all of that, plus delete, import, and users

   Schema is generic — one table of JSON rows keyed by
   (collection, id) — so adding a field never needs a migration.
   ============================================================ */
...
async function ensureSchema(sql) {
  ...
  /* First run: create the founding admin from environment variables so
     there is a way in before any user exists. */
  const count = await sql`SELECT COUNT(*)::int AS n FROM users`;
  if (count[0].n === 0) {
    const user = (process.env.ADMIN_USERNAME || 'admin').trim().toLowerCase();
    const pass = process.env.ADMIN_PASSWORD || process.env.APP_PASSWORD;
    if (pass) {
      const { salt, hash } = hashPassword(pass);
      await sql`
        INSERT INTO users (username, full_name, role, salt, hash, active)
        VALUES (${user}, ${process.env.ADMIN_NAME || 'Administrator'}, 'admin', ${salt}, ${hash}, true)
...
async function readAll(sql) {
  ...
  rows.forEach(r => {
    /* updated_by comes from the session, so it cannot be spoofed by the
       client. It overrides whatever the record's JSON happens to hold. */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && body.op === 'login') {
      ...
      /* Same message whether the user is missing, inactive, or the
         password is wrong — no probing for valid usernames. */
    ...
    if (req.method === 'POST') {
      ...
      if (op === 'changePassword') {
        ...
        await sql`
          UPDATE users SET salt = ${salt}, hash = ${hash}, must_change = false
      ...
      /* ---- data writes: both roles may create and edit ---- */
      if (op === 'upsert') {
      ...
      /* ---- deleting is destructive: admins only ---- */
      if (op === 'remove') {
      ...
      /* ---- bulk import rewrites many rows at once: admins only ---- */
      if (op === 'bulk') {
      ...
      if (op === 'updateUser') {
        ...
        if (body.role !== undefined && ROLES.includes(body.role)) {
          /* Never allow the last admin to be demoted — it would lock
        ...
        if (body.password) {
          ...
          await sql`
            UPDATE users SET salt = ${salt}, hash = ${hash}, must_change = true WHERE username = ${username}`;
