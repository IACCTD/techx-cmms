...
/* ---------- side drawer used for every create/edit form ---------- */
const Modal = {
  open({ title, body, footer }) {
    ...
    const first = document.querySelector('#drawerBody input,#drawerBody select,#drawerBody textarea');
...
const F = {
  ...
  num(name, label, val, opts = {}) {
    return `<label for="f_${name}">${esc(label)}</label>
      <input id="f_${name}" name="${name}" type="number" step="${opts.step || 'any'}"
        value="${val ?? ''}" ${opts.placeholder ? `placeholder="${esc(opts.placeholder)}"` : ''}/>`;
  },
  date(name, label, val) {
    return `<label for="f_${name}">${esc(label)}</label>
  ...
  /* read every named field inside the drawer */
  read() {
    const out = {};
...
function assetOptions(selected) {
  const list = DB.all('assets');
  const opts = [{ v: '', t: '— none —' }].concat(
    list.map(a => ({ v: a.id, t: a.id + ' · ' + a.name }))
  );
  return opts;
}
...
function renderTable(cols, rows, opts = {}) {
  if (!rows.length) {
    return `<div class="empty">${esc(opts.empty || 'Nothing here yet.')}</div>`;
  }
  ...
  const body = rows.map((r, i) => {
    ...
    const click = opts.onRow ? ` class="clk" onclick="${opts.onRow}('${esc(r.id)}')"` : '';
